...
const emptyBlocks = (): Record<MLBlockKey, string> =>
  Object.fromEntries(ML_BLOCKS.map((b) => [b.key, ''])) as Record<
    MLBlockKey,
    string
  >
...
export default function MLLab() {
  ...
  const [blocks, setBlocks] = useState<Record<MLBlockKey, string>>(emptyBlocks)
  const [running, setRunning] = useState(false)
  const [runResult, setRunResult] = useState<RunResult | null>(null)
  const [coachOutput, setCoachOutput] = useState('')
  const [coachRunning, setCoachRunning] = useState(false)

  const preset: MLPreset | null =
    ML_PRESETS.find((p) => p.key === presetKey) ?? null
  const isOwn = presetKey === 'own'
  const csv = isOwn ? ownCsv : preset?.csv ?? ''
  const examples = preset?.examples ?? ML_GENERIC_EXAMPLES
  const question = preset
    ? preset.question
    : 'Your own mission: pick something your data could predict.'
  const preview = useMemo(() => parseCsvPreview(csv), [csv])

  const filledCount = ML_BLOCKS.filter((b) => blocks[b.key].trim()).length
  ...
  const script = useMemo(() => {
    ...
    ML_BLOCKS.forEach((b, i) => {
      const value = blocks[b.key].trim()
      if (value)
        parts.push(`# ${i + 1} · ${b.label} — ${b.topic.toLowerCase()}\n${value}`)
    })
    return parts.join('\n\n')
  }, [blocks])

  const setBlock = (key: MLBlockKey, value: string) =>
  ...
  const chooseData = () => {
    if (!csv.trim() || !preview) {
      toast({
        status: 'warning',
        title: isOwn
          ? 'Paste some CSV first — a header line plus a few dozen rows.'
  ...
  const runScript = async () => {
    const missing = ML_BLOCKS.filter((b) => !b.optional && !blocks[b.key].trim())
    if (missing.length > 0) {
      toast({
        status: 'warning',
        title: 'A few blocks are still empty',
        description: `Fill in: ${missing.map((b) => b.label).join(', ')}.`,
      })
      return
    }
    setRunning(true)
  ...
  const buildCoachPrompt = () => {
    ...
    return `Task context: You are a friendly machine learning coach on the last afternoon of a machine learning training for beginners. The course covered four topics with scikit-learn: regression, classification, clustering and ensemble methods (plus the Python basics: pandas and matplotlib). The participant just built their first complete ML script out of building blocks.
...
` : ''}## Did the models answer the mission?
One honest, encouraging paragraph — mention the actual metrics they got (MAE, R², accuracy) and whether those are good for this data.
## What each part does
One bullet per course topic the script actually uses (regression, classification, clustering, ensemble methods) — one plain-language sentence each about THEIR code and THEIR results, not generic theory. Include a sentence on the train/test split if they used one.
## One thing to try next
One small, concrete improvement with a short code snippet — for example another feature, a different max_depth or n_clusters, or a metric they haven't seen yet.
  ...
  return (
    <Box h="100%" overflowY="auto" px={{ base: 6, lg: 10 }} py={8}>
      <Box maxW="1100px" mx="auto">
        <Heading fontSize="2xl">The ML capstone: four models, one dataset</Heading>
        <Text color="gray.500" mt={1}>
          Regression · Classification · Clustering · Ensemble methods —
          everything from the training on one dataset, in one script.
        </Text>
        <Stepper
          ...
          <Box>
            <SimpleGrid columns={{ base: 1, md: 2 }} spacing={3}>
              ...
              <PresetCard
                ...
                description="Paste a small CSV — a few numeric columns and something to predict."
                selected={isOwn}
                dashed
            ...
            {isOwn && (
              <Textarea
                ...
                fontSize="xs"
                spellCheck={false}
                placeholder={'size,age,category,price\n120,15,B,410000\n…'}
                value={ownCsv}
        ...
        {step === 'build' && (
          <Box>
            <Callout tone="guide" title="Build it block by block 🧱">
              Each block is one ML topic from the training. Type real Python —
              your script grows along on the right. Stuck? Hover “example” for a
              snippet that fits this dataset, or click it to insert.
            </Callout>
              ...
              >
                <Flex justify="space-between" align="center">
                  ...
                  <Text fontSize="xs" color="gray.400" fontFamily="body">
                    {filledCount} / {ML_BLOCKS.length} blocks
                  </Text>
                </Flex>
                <Progress
                  value={(filledCount / ML_BLOCKS.length) * 100}
                  size="xs"
                  colorScheme="purple"
                  borderRadius="full"
                  bg="gray.700"
        ...
        {step === 'run' && runResult && (
          <Box>
            ...
            {runResult.timed_out && (
              <Box mt={4}>
                <Callout tone="warn" title="Stopped after the time limit ⏱">
                  ...
                  seconds. Training on this data takes a second or two — check
                  for an endless loop, or an accidentally huge n_estimators.
                </Callout>
              </Box>
            )}
            ...
            {hasError && !runResult.timed_out && (
              <Box mt={4}>
                <Callout tone="warn" title="It crashed — that's part of the job 🔍">
                  Every ML practitioner reads tracebacks daily. Start at the
            ...
            >
              ...
              <Box display="flex" flexDirection="column" gap={4}>
                <OutputCard label="What it printed — your metrics" mono>
                  {runResult.stdout ||
                    '(nothing printed — print your MAE, R² and accuracy so you can judge the models)'}
                </OutputCard>
                {runResult.stderr && (
                ...
                {runResult.image_base64 ? (
                  ...
                  !hasError && (
                    <Text fontSize="xs" color="gray.500">
                      No chart this run — the last block draws one.
                    </Text>
                  )
                )}
              </Box>
            ...
            <Flex align="center" gap={3}>
              ...
              <Text fontSize="sm" color="gray.500">
                {hasError
                  ...
                  : 'Get a review: are your metrics any good, what does each model say, and one thing to try next.'}
              </Text>
            </Flex>
            {!coachOutput && (
              <Button
